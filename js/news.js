...
class PremiumNewsHub {
    ...
    setupDebugPanel() {
        ...
        debugPanel.id = 'news-debug-panel';
        ...
        debugPanel.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                <strong>News API Debug</strong>
                <button onclick="this.parentElement.parentElement.style.display='none'" style="background: #0f0; color: #000; border: none; padding: 2px 8px; cursor: pointer;">✕</button>
            </div>
            <div id="news-debug-content"></div>
        `;
        document.body.appendChild(debugPanel);
    }
    
    updateDebugPanel() {
        const debugContent = document.getElementById('news-debug-content');
        ...
        if (this.debug.lastRequest) {
            html += `<div><strong>Last Request:</strong><pre style="margin: 4px 0; white-space: pre-wrap;">${JSON.stringify(this.debug.lastRequest, null, 2)}</pre></div>`;
        }
        
        if (this.debug.lastResponse) {
            html += `<div><strong>Last Response:</strong> ${this.debug.lastResponse.articles || 0} articles, Status: ${this.debug.lastResponse.status || 'N/A'}, Page: ${this.debug.lastResponse.page || this.currentPage}/${this.debug.lastResponse.totalPages || this.totalPages}</div>`;
        }
        
        if (this.debug.lastError) {
            html += `<div style="color: #f00;"><strong>Last Error:</strong> ${this.debug.lastError.message || 'N/A'} (${this.debug.lastError.status || 'N/A'})</div>`;
        }
        
        html += `<div><strong>Current:</strong> Page ${this.currentPage}/${this.totalPages}, Articles: ${this.allArticles.length}, HasMore: ${this.hasMore}</div>`;
